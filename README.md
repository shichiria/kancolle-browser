...
- **艦隊ダッシュボード** — 全4艦隊のリアルタイム表示（HP・燃料・弾薬・士気）、司令部施設表示
- **遠征チェッカー** — 成功/大成功の判定とカウントダウンタイマー、帰還1分前の画面通知
- **出撃任務チェッカー** — 314以上の任務に対応した編成条件の自動判定
- **海域ルート推奨編成** — 通常海域（1-1〜7-5）のルート制御用推奨編成チェッカー
- **戦闘ログ** — HP/ダメージ・陣形・制空値の記録とマップルート可視化
- **ミニマップ** — ゲーム画面オーバーレイでマップルートをリアルタイム表示（ボス表記対応）
- **装備改修トラッカー** — 曜日別の改修可否カレンダーと改修履歴、消費装備表示
- **艦娘・装備一覧** — フィルタ付きの艦娘リストと装備リスト
- **Google Drive 同期** — 任務進捗・改修履歴・戦闘ログ・戦果データのクラウドバックアップ（任意）
...
Google Drive同期を含むビルドでは、OAuthデスクトップクライアントの値を
`KANCOLLE_GOOGLE_CLIENT_ID` と `KANCOLLE_GOOGLE_CLIENT_SECRET` の環境変数で
ビルド時に指定してください。値を変更した場合はアプリを再ビルドし、漏えい時は
Google Cloud Consoleでクライアントをローテーションしてください。
...
- `sync/raw_api/*.json` — 設定で明示的に有効化した場合の艦これAPIリクエスト/レスポンス（起動時はOFF）
